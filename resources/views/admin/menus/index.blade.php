...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            //Auto Refresh Ajax code
            function reloadSidebar() {
                $.ajax({
                    url: "/ajax/sidebar",
                    method: "GET",
                    success: function(html) {
                        $("#sidebar-nav").parent().html(
                            $(html).find("#sidebar-nav").parent().html()
                        );
                    },
                    error: function() {
                        console.error("Failed to reload menu.");
                    }
                });
            }
            ...
            $('#menuForm').submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if ($('#tree-view').is(':visible')) loadMenuTree(); // reload tree
                        toastr.success(response.message); // green success
                    },
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            // Validation errors
                            ...
                            $.each(errors, function(key, value) {
                                ...
                                field.siblings('.invalid-feedback').text(value[0]).css(
                                    'color', 'red'); // RED errors
                                toastr.error(value[0]); // show toast too
                            });
                        } else {
            ...
            $(document).on('click', '.delete-btn', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        table.ajax.reload();
                        if ($('#tree-view').is(':visible')) loadMenuTree();
                        toastr.success(response.message); // green success
                    },
                    error: function(xhr) {
                        toastr.error(xhr.responseJSON?.message ||
                        'Error deleting menu'); // red error
                    }
                });
            });
        });
