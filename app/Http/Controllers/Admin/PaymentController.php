...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $centers = DB::table('center_candidate')
            ->select(
                [
                    ...
                    'centers.center_name',
                    'centers.district',
                    DB::raw("count(DISTINCT candidate_no ) as candidates"),
        ...
        if ($request->ajax()) {
            return   DataTables::of($centers)
                ->addColumn('actions', function ($model) {
                    ...
                    $actions .= "<a href='" . route('admin.bankstatement.bankstatement', $model->center_no) . "'
                            data-toggle='tooltip' title='Bank Statements'
        ...
        try {
            ...
            // return view('admin.finance.payments', compact('nmds', 'mosd', 'other', 'total_sum'));
        } catch (\Exception $e) {
            dd($e);
        }
    }

    public function paymentPrivateCenters(Request $request)
    {
        if ($request->ajax()) {
            $privateCenters  = DB::table('center_candidate')
                ->select(
                    [
                        'center_candidate.center_no',
                        'centers.center_name',
               ...
               order by center_candidate.session separator ',') as sessions"),
                ...
                ->groupBy('center_candidate.center_no');
            return   DataTables::of($privateCenters)
                ->addColumn('total', function ($model) {
                    $schoolResult = Payment::privatefees($model->center_no);
                    $total = $schoolResult['sponsor'][0];
                    return  "LSL " . number_format($total, 2, '.', '');
                })
                ->make(true);
        }
    }
    ...
    {
        $centerFee = new Payment();
        $financial_year = (date('m') <= 2) ? (date('Y') - 1) . '-' . date('Y') : date('Y') . '-' . (date('Y') + 1);
        if ($request->candidate_type == 1) {
            return  $centerFee->exportschoolfees(null,  null, 'November', $financial_year);
        } else {
            return  $centerFee->exportsprivatefees();
        }
    }
